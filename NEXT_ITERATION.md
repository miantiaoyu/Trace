...
- 确认公司 Linux 测试服务器、镜像仓库和 CI/CD 平台归属。
...
3. 安装 systemd unit，手动启动一次 `trace.service`，核对退出码、任务锁、脱敏日志和 `headway` 更新后再启动 `trace.timer`。
4. 观测 HMM 120 秒等待与 60–75 秒重试退避的恢复率；万海仍受 Incapsula 影响时保持失败重查，不绕过访问控制。
5. 测试运行稳定后，再决定 systemd 执行频率、单轮 `limit` 和正式目标库发布时间。
...
- Docker/systemd 日志不包含柜号、官网原始响应或数据库凭证。
