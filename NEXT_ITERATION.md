...
4. 上传包含低频策略和浏览器会话复用的新服务器包，观测全量任务的开始/结束时间、各船司成功率、Worker 内存和次日是否发生任务锁冲突。
5. 向 HMM 申请公司服务器固定公网出口 IP 白名单或正式 Track & Trace 接口；万海仍受 Incapsula 影响时保持失败重查，不绕过访问控制。
6. 测试运行稳定后，再决定正式目标库发布时间；当前仍保持每天一次、单轮 `limit=0`。
...
- Linux + Xvfb 能无人值守运行浏览器路线；同船司复用会话，单柜超时会重建 Worker 且不中断后续柜号。
