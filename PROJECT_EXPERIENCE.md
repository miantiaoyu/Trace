...
- 集装箱号会被重复使用。历史数据库记录对应的柜号在官网可能已经显示新一票运输，不能只凭数据库 `update_time` 判断官网结果属于原订单；阶段验证必须检查事件时间和运输状态。
- ONE 的 Actual/Estimated 语义存在于事件时间图标：蓝色 A 对应 `#00506D`，粉色 E 对应 `#BD0F72`。地点、码头和船名只在阶段首行出现，后续事件要继承上下文，但陆侧事件不能错误继承船舶模式。
- 阳明应优先使用 `dcsaStatusInfo`，其中 `eventClassifie` 明确给出 Actual/Estimated；`tsMode` 可能使用 `<BR />` 拼接运输方式、船名和航次。
- HTML 表格解析应统一使用能容忍不完整标签的 DOM 解析器，并在解析后继续执行表头和业务契约校验。HMM、长荣和万海已统一使用 Selectolax；解析器能修复 HTML 不代表页面业务结构仍然有效。
- 定时任务稳定性需要同时覆盖单柜隔离与整轮保护：独立子进程负责硬超时，文件锁阻止任务重叠，脱敏日志记录成功率与耗时，连续失败状态用于发现官网长期改版或封禁。
- 瞬时错误重试不能固定间隔。应使用指数退避和随机抖动，并与同船司最低访问间隔取较大值；访问拒绝、验证码和页面契约变化不是瞬时故障，不应通过重试放大请求量。
...
- Yang Ming: 公开 JSON 接口可直接按柜号调用；微运给出的旧页面路径需要修正。`dportETA` 可能是 `ETA at <port>: YYYY/MM/DD HH:MM`，写入 `DATETIME` 前必须去除说明前缀。
- Evergreen: 官方中国站的公开页面可直接 `POST` 到 `TDB1_CargoTracking.do`。柜号查询参数为 `TYPE=CNTR`、`SEL=s_cntr`、`CNTR=<柜号>`、`NO=<柜号>`；真实 HTML 可能把页面说明和数据拼入同一表头单元格，数据行仍为固定 8 列。应用已知业务表头对数据行建模，并从页面文本单独提取预计抵达时间。
...
- 已确认可自动化：Yang Ming、SM Line、COSCO、ONE、Evergreen、Maersk、MSC、HMM（有界浏览器）。
- 路线已验证但当前受访问前置校验影响：Wan Hai（Incapsula）。
