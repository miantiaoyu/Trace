...
- 已验证马士基（Maersk）官网追踪页：可在同船司批次复用的浏览器会话中获得页面自身请求的完整原始 JSON，包含起讫港、集装箱事件、船名航次、状态与 ETA。
...
- 子进程 Provider 失败会保留真实异常类型到 `last_error_type`，例如 `HmmTrackingError`、`WanHaiTrackingError`，避免批量失败都显示为外层 `RuntimeError`。
- 有限 `--limit` 时已到期的重查记录优先于 ERP 新单，避免失败记录在持续新单下长期饥饿；HMM 等待追踪响应 120 秒，第二次尝试使用 180–240 秒退避。
- 支持完整船司字段归一化，已覆盖数据库常见的中英文、简称、BCO/NVO 写法；HMM 对历史乱码后缀使用 `HMM%` 数据库前缀兜底。
- 统一入口按船司路线执行最低访问间隔、硬超时和有限重试；批次先按船司分组，同一船司复用一个 Provider 子进程，浏览器路线同时复用 Playwright、Chromium、Cookie 和页面，分组结束后关闭进程并恢复 ERP 原结果顺序。单柜超过硬超时会终止当前 Provider 子进程，下一次尝试重新创建，查询、路由和归一化异常不会终止整个批次。
- 默认最低访问间隔已提高为：阳明/SM Line/长荣 30 秒，COSCO/ONE 60 秒，马士基/MSC 90 秒，万海/HMM 120 秒。全量批次允许运行数小时，由任务锁阻止次日任务与上一轮重叠。
...
- 2026-07-20 本机公网出口 IP 被 HMM 防火墙明确拦截，同一 IP 下会在“防火墙拦截页”和 120 秒无响应之间波动；程序会在识别到拦截文案时记录明确的 `HmmTrackingError`。长期运行需申请固定出口 IP 白名单或正式接口，不采用随机代理绕过访问控制。
...
- 2026-07-20 Docker 回归验证常驻 Provider Worker 可连续完成 2 条阳明查询并正常退出；HMM 单柜在当前出口 IP 下返回真实 `HmmTrackingError`，不再被外层异常类型覆盖。
...
| 船司 | 订单量 | 去重柜量 | 官网路线状态 |
| --- | ---: | ---: | --- |
| HMM | 417 | 416 | 路线已实现；当前出口 IP 被防火墙拦截 |
| 马士基 | 225 | 225 | 可获取 |
| 万海 | 63 | 63 | 可获取 |
