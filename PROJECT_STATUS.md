...
- 统一摘要由 Pydantic 数据契约校验，当前 schema 为 `1.1`；未知字段、非法事件时间类型和缺失必填标识会在归一化边界明确失败，不会静默污染下游数据。
...
- 测试配置模板使用 `oms` 数据库；当前结果仍只输出报告，不创建结果表、不写公司数据库。
...
- 结果落库仍未实现，必须先确定表结构、写入权限、去重键和保留策略。
