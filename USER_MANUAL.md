...
批次会先按船司分组查询，再把结果恢复成 ERP 原顺序。同一船司在一轮任务内复用一个 Provider 子进程；COSCO、ONE、马士基、MSC、HMM 会复用 Playwright、Chromium、Cookie 和页面，万海会复用预热得到的 Cookie 与 ViewState。分组结束后进程和浏览器立即关闭，不会常驻服务器。单柜超过硬超时后父进程会终止当前 Worker，重试时重新创建，因此浏览器崩溃或页面卡死不会阻塞整个批次。

## 默认限速与重试

| 路线 | 最小间隔 | 单次硬超时 | 最大尝试次数 |
| --- | ---: | ---: | ---: |
| 阳明、SM Line、长荣 | 30 秒 | 45 秒 | 2 |
| COSCO、ONE | 60 秒 | 60 秒 | 2 |
| 马士基、MSC | 90 秒 | 90 秒 | 2 |
| 万海 | 120 秒 | 120 秒 | 1 |
| HMM | 120 秒 | 240 秒 | 2 |

只有可恢复错误才重试，包括硬超时、连接中断、浏览器访问失败、HTTP 429 和 HTTP 5xx。柜号无数据、页面契约变化、参数错误、验证码或访问拒绝不会重试。默认退避从 2 秒开始、最大 30 秒，并且下一次开始仍必须满足路线最小间隔；HMM 等待追踪响应 120 秒，首次失败后等待 180–240 秒再做第二次尝试。没有无限重试，实际等待时长记录在 `execution.retry_delays_seconds`。

低频全量任务可能运行数小时，这是每天一次定时任务的预期行为。任务锁会阻止两个批次同时运行；不要为了缩短时间启用并发，也不要在正式定时命令中用 `--min-interval-seconds` 覆盖这些默认值。
...
如果页面显示 `Your access is blocked by our firewall`，程序会记录“HMM 防火墙已拦截当前公网出口 IP”。同一出口也可能直接等待 120 秒后超时。这不是 Docker 或 Xvfb 故障，需要用公司固定公网出口 IP 向 HMM 申请解封/白名单，或申请正式接口。
