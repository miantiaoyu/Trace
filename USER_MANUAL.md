...
会生成 `dist/trace-server.zip`。压缩包只包含运行源码、Docker 文件、systemd unit、启动脚本和服务器说明；不包含 `tests/`、`docs/`、`crawler_lab/`、`sql/`、Git 元数据、日志或真实数据库配置。服务器解压后需单独放入 `prod-db.yml` 和 `test-db.yml`。
...
服务器使用 systemd timer 调度 Docker 一次性容器，不需要 Python 常驻、不监听端口。`deploy/run-trace.sh` 固定读取 `prod-db.yml` 中的阿里正式 ERP，写入 `test-db.yml` 中的内网测试 `oms.headway`，并只输出脱敏汇总。
